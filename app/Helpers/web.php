...
function menuActiveClass($active = null, $type = null) {
    ...
    $lastarray = end($currentPathAry);
    $prearray = prev($currentPathAry);

    if($lastarray == 'index'){
        $controller = $prearray;
    }else{
        $controller = $lastarray;
    }
...
function calculateMySqlDateOnly($date) {
...
function showStandardDate($date) {
    if ($date) {
...
function getClientId($request) {
    ...
    if ($client) {
        ...
        $data = array(
            ...
            'email' => $request->email,
            'phone' => $request->phone,
            'dob' => calculateMySqlDateOnly($request->dob),
...
function showTotalOutstanding($datemonth, $data, $position) {
    
    $currentPosition = $datemonth[$position];
   
    if (isset($data[$currentPosition])) {        
        return $data[$currentPosition]['total'];
...
function getRepedDate($data) {
    ...
    $bookingDates[] = $booking_time; //$data->booking_time;
    ...
    if ($data->repeat_type == 2){        
        ...
        $endDate = $data->repeat_booking_endtime ? calculateMySqlDate($data->repeat_booking_endtime) : \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $booking_time)->addMonths(2)->toDateTimeString();
        ...
        foreach ($data->repet_days as $key => $repet_days) {
            $repetDate = \Carbon\Carbon::parse($booking_time)->modify("next $repet_days")->format('Y-m-d');
        ...
        $repeat_booking_time = [];
        if($data->repeat_booking_time){
            foreach($data->repeat_booking_time as $repeatTime){
                
                $r_dates = explode(" ", $repeatTime);
                
                //$repeat_time = \Carbon\Carbon::parse($aa[0])->format('Y-m-d');

                $repeat_booking = explode("/", $r_dates[0]);

                $repeat_booking_time[] = $repeat_booking[2]. '-'. $repeat_booking[0]. '-'. $repeat_booking[1]. ' '. $r_dates[1]; 

                //echo \Carbon\Carbon::parse($repeatTime)->format('Y-m-d');
            }
        }

        //$bookingDates = array_merge($data->repeat_booking_time, $bookingDates);
        $bookingDates = array_merge($repeat_booking_time, $bookingDates);
    }
...
function get_editable_value($value, $default=''){
